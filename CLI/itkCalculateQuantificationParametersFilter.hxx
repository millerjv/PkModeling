...
::ThreadedGenerateData( const typename Superclass::OutputImageRegionType& outputRegionForThread,
  ...
  while (!inputVectorVolumeIter.IsAtEnd() )
    ...
    //	//std::string voxelInfo =
    // "D:/Codes/Slicer4/Modules/CLI/PkModeling/Data/DCEMRIData/voxel_AUCLessThan0_"
    // +iterStringx.str()+std::string("_")+iterStringy.str()+"_"+iterStringz.str()+std::string(".txt");

    //	FILE* fp = fopen(voxelInfo.c_str(),"w");
    //	float tempAUCNumber = PkSolver::area_under_curve(m_timeSize, m_timeAxis,
    // const_cast<float *>(vectorVoxel.GetDataPointer()), BATIndex,
    // m_AUCTimeInterval);
    ...
    //		std::cout<<"i,
    // concentration:"<<itemp<<","<<tempConcentration[itemp]<<std::endl;
    //	}
    //	fprintf(fp,"%f\n",tempAUCNumber);
  ...
  VectorVolumePointerType inputVectorVolume, VolumeConstPointerType inputVolume,float*& averageAIF)
{
  ...
  int             numberVoxels = 0;
  //float * averageAIF = new
  // float[(int)inputVectorVolume->GetNumberOfComponentsPerPixel()]();
  ...
  while (!inputVectorVolumeIter.IsAtEnd() )
    ...
    if (inputVolumeIter.Get()!=0) //Pixel value >0 will be consider to be a
      ...
      //std::cout << "volumeIndex:"<<
      // volumeIndex[0]<<","<<volumeIndex[1]<<","<<volumeIndex[2]<< std::endl;
      ...
      //std::string voxelInfo =
      // "D:/Codes/Slicer4/Modules/CLI/PkModeling/Data/DCEMRIData/voxel_"
      // +iterStringx.str()+std::string("_")+iterStringy.str()+"_"+iterStringz.str()+std::string("_SI.txt");
      //FILE* fp = fopen(voxelInfo.c_str(),"w");

      numberVoxels +=1;
      //std::cout << "numberVoxels:"<< numberVoxels << std::endl;
      vectorVoxel = inputVectorVolumeIter.Get();
      //std::cout << "getdataPointer"<< vectorVoxel.GetDataPointer()[9] <<
      // std::endl;
      for(int i = 0; i < (int)inputVectorVolume->GetNumberOfComponentsPerPixel(); i++)
        ...
        averageAIF[i]+=(float)vectorVoxel.GetDataPointer()[i];    //not an
                                                                  // average
                                                                  // value
  ...
  //std::string voxelAveInfo =
  // "D:/Codes/Slicer4/Modules/CLI/PkModeling/Data/DCEMRIData/voxel_average_SI.txt";
...
::SetTimeAxis(vcl_vector<float> inputTimeAxis)
{
  ...
  for(int i=0; i<(int)inputTimeAxis.size(); ++i)
    ...
    //std::cerr << std::endl << "timeAxis[tempIndex]:"<< m_timeAxis[i] <<
    // std::endl;
    }
}
